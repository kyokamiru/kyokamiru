...
- 本番管理画面から「めっちゃカメレオン」を登録し、`/games` への即時掲載、詳細ページの HTTP 200、免責・根拠 URL・最終確認日の表示を確認した。再保存後に `/sitemap.xml` と詳細ページがキャッシュ MISS で再生成され、サイトマップへのゲーム URL 掲載も確認した。これにより on-demand revalidation と Zone Cache Purge の本番動作を確認済み。
- 残対応: 一般公開前に CPU を最適化し、P90 が 10 ms を超える場合は Workers Paid へ移行する。

## Phase 7: UX 改善 v2(2026-07-15 フィードバック反映)

本番公開後のレビューで挙がった 3 点を解消する。仕様の正は screens.md / database.md / steam-integration.md の v2 記述。

### 7-1. DB・取込の拡張(先行して行う)

- 追加マイグレーション(database.md「適用済み DB への追加マイグレーション(v2)」): `play_modes` / `screenshots` / `movie_url` / `movie_thumbnail_url` / `summary`
- 型再生成
- Steam 取込の拡張(steam-integration.md §2): categories→play_modes 変換、screenshots・movies の取得。**説明文は取り込まない**
- 管理画面フォーム拡張(screens.md §7): スクリーンショット取捨選択 UI、プレイスタイル修正、summary 入力

### 7-2. トップページの刷新

- ヒーロー(テキスト+検索ボックス)を廃止し、`HeroCarousel`(大型 KV、新着 5 件)に置き換え(screens.md §1)
- **検索ボックスをヘッダーの 1 箇所に一本化**(重複解消)
- キャッチコピーはカルーセル上部のセクション見出しに縮小

### 7-3. 一覧のプレイスタイル絞り込み

- `mode` クエリパラメータ(カンマ区切り複数、OR 条件)+サイドバーのチェックボックス(screens.md §2)
- GameListRow にプレイスタイルアイコン表示

### 7-4. 詳細ページのメディアギャラリー

- `MediaGallery`(動画先頭+スクリーンショット、Steam CDN ホットリンク、`preload="none"`)をページ最上部の左カラムに配置(screens.md §3)
- summary(あれば)をタイトル直下に表示

**受け入れ条件**:
- トップに検索ボックスが 1 つだけ存在し、ファーストビューの主役がゲームビジュアルになっている
- 既存ゲーム(App ID 4704690)を管理画面で再取込 → スクリーンショット・動画・プレイスタイルがプリフィルされ、公開ページのギャラリーに反映される
- `/games?mode=singleplayer` 等の絞り込みが機能し、選択条件がチップ表示される
- メディア未設定の既存ゲームでも詳細ページが崩れない(header_image フォールバック)
- 動画はユーザーが再生操作するまで Steam CDN へリクエストが飛ばない
- モバイル(375px)で可否サマリーがスクロールなしで見える
- `pnpm lint` / `pnpm typecheck` / `pnpm build` / `pnpm preview` がすべて通り、Workers 上で表示確認できる
